...
saveShipping = async (req, res) => {
    ...
    try {
        const result = await db.query(
            "INSERT INTO shipping_addresses (user_id, address, city, state, country, zip_code, phone_number) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id",
            [user_id, address, city, state, country, zip_code, phone_number]
        );
        res.status(201).json({ message: "Shipping saved", shippingId: result.rows[0].id });
    } catch (error) {
